...
## VET-171 — На форме выбора клиники кнопка «Войти» срабатывает не с первого раза

**ID артефакта:** VET-171  
**Статус:** открыт  
**Модуль:** экран входа / выбор клиники (форма выбора клиники при входе в приложение)

### Описание

При входе в приложение на форме выбора клиники кнопка «Войти» не срабатывает с первого нажатия — требуется повторное нажатие.

### Где воспроизвести

- Запустить приложение → на форме выбора клиники нажать «Войти» один раз → переход не выполняется; при повторном нажатии кнопка срабатывает.

### Отладочная информация

В логе (Android) при нажатии наблюдается:
- `WindowOnBackDispatcher: sendCancelIfRunning: isInProgress=false callback=io.flutter.embedding.android.FlutterActivity$1@...`
- Сообщения VRI `setFrameRateCategory` (touch hint / boost timeout).

Фрагмент лога: `terminals/2.txt`, строки 1016–1024 (или аналог в окружении пользователя).

### Связанные файлы

- Экран/виджет формы выбора клиники при входе (предположительно: клиника, логин, onboarding или главный роутер).

---

<a id="vet-172"></a>

## VET-172 — Кнопка «Вход администратора» нажимается не с первого раза

**ID артефакта:** VET-172  
**Статус:** открыт  
**Модуль:** экран входа / главный экран (кнопка перехода в админку)

### Описание

Кнопка «Вход администратора» не срабатывает с первого нажатия — требуется повторное нажатие.

### Где воспроизвести

- Запустить приложение → на главном экране (или экране выбора клиники) нажать «Вход администратора» один раз → переход не выполняется; при повторном нажатии кнопка срабатывает.

### Отладочная информация

В логе (Android) при нажатии наблюдается:
- `InsetsController: cancelAnimation: types=ime` — отмена анимации клавиатуры
- `ImeTracker: onShown` — клавиатура показана
- `FlutterJNI: Sending viewport metrics to the engine` — обновление метрик viewport
- Сообщения VRI `setFrameRateCategory` (touch hint / boost timeout)

Фрагмент лога: `terminals/2.txt`, строки 943–959 (или аналог в окружении пользователя).

### Связанные файлы

- Экран с кнопкой «Вход администратора» (главный экран, список пациентов или форма выбора клиники); роутер перехода в админку.

---

<a id="vet-173"></a>

## VET-173 — При открытии формы редактирования табличного раздела открывается пустая форма

**ID артефакта:** VET-173  
**Статус:** исправлен  
**Модуль:** `lib/features/admin/presentation/widgets/table_section_edit_page.dart` — форма редактирования раздела «Таблица»

### Описание

При открытии формы редактирования табличного раздела отображается пустая форма. В логе — исключения layout: «BoxConstraints forces an infinite width», затем каскад «RenderBox was not laid out» (в т.ч. _RenderListTile).

### Причина

В виджете `_TableCellEditor` первая строка — `Row`, в которой стоят `CheckboxListTile` и `DropdownButton` (или `SizedBox` с полем) без `Expanded`/`Flexible`. В `Row` дочерние виджеты без гибкости получают неограниченную ширину; `ListTile` (внутри `CheckboxListTile`) при этом получает infinite width и ломает layout, из‑за чего форма не отрисовывается.

### Где воспроизвести

- Админка → Редактирование шаблона протокола → раздел типа «Таблица» → открыть раздел на редактирование → форма пустая, в консоли — исключения layout.

### Отладочная информация

Фрагмент лога: `terminals/2.txt`, строки 913–1020.  
Типичные сообщения: `BoxConstraints forces an infinite width`; `RenderBox was not laid out: _RenderListTile#...`; `Failed assertion: '!_debugDoingThisLayout': is not true`.

### Исправление

В `_TableCellEditor` первая `Row` перестроена: `CheckboxListTile` и блок с `DropdownButton`/полем статичного текста обёрнуты в `Expanded`, чтобы они получали ограниченную ширину внутри `Row` и не приводили к infinite width для `ListTile`.

### Связанные файлы

- `lib/features/admin/presentation/widgets/table_section_edit_page.dart` (виджет `_TableCellEditor`)

---

<a id="vet-174"></a>

## VET-174 — При открытии визуального редактора с табличным разделом — ошибка Invalid argument(s): 4.0

**ID артефакта:** VET-174  
**Статус:** исправлен  
**Модуль:** `lib/features/admin/presentation/widgets/print_layout_editor_page.dart` — визуальный редактор печатной формы, блок таблицы

### Описание

При открытии визуального редактора расположения на странице, если в шаблоне есть табличный раздел, возникает исключение: «Invalid argument(s): 4.0». Та же ошибка может возникать при редактировании количества строк и столбцов в редакторе табличного раздела (форма редактирования раздела «Таблица»): при изменении полей «Строк»/«Столбцов» и последующем открытии визуального редактора или при пересчёте размеров блока таблицы с малыми суммарными размерами.

### Причина

В методе `_buildTableBlockContent` для размеров ячеек и столбцов/строк использовался вызов `value.clamp(4.0, totalW)` (и аналогично для высоты). В Dart у `clamp(lowerLimit, upperLimit)` требуется `lowerLimit <= upperLimit`. Когда `totalW` или `totalH` (или `availableW`/`availableH`) меньше 4.0 (например, блок ещё мал, размеры в мм нулевые или после уменьшения числа строк/столбцов), получается недопустимый диапазон и выбрасывается ошибка «Invalid argument(s): 4.0».

### Где воспроизвести

- Админка → Редактирование шаблона протокола → раздел типа «Таблица» → кнопка «Расположение на странице» (визуальный редактор) → при отрисовке блока таблицы ошибка в консоли.
- Тот же сценарий после изменения количества строк или столбцов в форме редактирования табличного раздела (поля «Строк», «Столбцов») и повторного открытия визуального редактора или при малых размерах блока таблицы.

### Отладочная информация

Фрагмент лога: `terminals/2.txt`, строки 1020–1024. Сообщение: `Another exception was thrown: Invalid argument(s): 4.0`.

### Исправление

Для всех вызовов `.clamp(4.0, upperBound)` в блоке таблицы верхняя граница заменена на `math.max(4.0, upperBound)`, чтобы диапазон всегда был допустим: ширина/высота столбцов и строк — `clamp(4.0, math.max(4.0, availableW/H))`, размеры ячеек в `Positioned` — `clamp(4.0, math.max(4.0, totalW/totalH))`.

### Связанные файлы

- `lib/features/admin/presentation/widgets/print_layout_editor_page.dart` (`_buildTableBlockContent`)

---

<a id="vet-177"></a>

## VET-177 — В форме редактирования протокола не редактируются редактируемые ячейки табличного раздела

**ID артефакта:** VET-177  
**Статус:** исправлен  
**Модуль:** форма создания/редактирования протокола, табличный раздел (TemplateFormBuilder, ExaminationCreatePage)

### Описание

При открытии существующего протокола на редактирование поля ввода ячеек табличного раздела не отображают сохранённые значения и/или не реагируют на ввод — ячейки ведут себя как нередактируемые.

### Причина

Форма строится до загрузки данных протокола: шаблон подставляется первым, а `_formValues` заполняются в `_initializeFromExam` в `addPostFrameCallback`. У `TextFormField` с `initialValue` значение используется только при создании состояния (initState). При последующей пересборке с уже заполненным `values` Flutter переиспользует существующее состояние поля и не обновляет текст по новому `initialValue`, поэтому ячейки таблицы остаются пустыми и не синхронизируются с данными.

### Где воспроизвести

- Открыть существующий протокол на редактирование, в шаблоне которого есть табличный раздел с полями ввода → ячейки пустые или не редактируются.

### Исправление

1. Пересоздание формы после инициализации данных: для блока формы передаётся ключ, зависящий от `_initializedForEdit`; для ячеек таблицы задан уникальный `ValueKey` по позиции (строка, столбец).
2. **Дополнительно (надёжная подстановка значений):** поля ввода ячеек таблицы вынесены в виджет `_TableCellInputField` с `TextEditingController`. В `initState` контроллер инициализируется из `value`; в `didUpdateWidget` при изменении значения извне (загрузка протокола) обновляется `controller.text`. Так сохранённые значения гарантированно отображаются после загрузки и поля остаются редактируемыми.

### Связанные файлы

- `lib/features/examinations/presentation/pages/examination_create_page.dart` (_TemplateFormSection, передача ключа формы)
- `lib/features/templates/presentation/widgets/template_form_builder.dart` (_buildTableSection, _TableCellInputField)

---

<a id="vet-178"></a>

## VET-178 — В печатной форме протокола не отображаются табличные разделы

**ID артефакта:** VET-178  
**Статус:** закрыто  
**Модуль:** `lib/features/pdf/services/protocol_pdf_service.dart` — генерация PDF протокола

### Описание

В печатной форме (PDF) протокола табличные разделы не отображаются. Они должны отображаться независимо от того, заполнены данные в ячейках или нет.

### Причина

1. В режиме с позиционированием разделов (`_buildPositionedDataPage`, `_buildPositionedSectionsForPage`) в цикле по разделам использовались только `section.fields` и «есть ли данные»; у табличных разделов нет `fields`, а контент в `tableConfig`, поэтому такие разделы пропускались (`sectionEntries.isEmpty` / `hasContent`).
2. В одностраничном потоке блок «Данные осмотра» (в т.ч. таблицы) добавлялся только при `examination.extractedFields.isNotEmpty`, поэтому при пустом протоколе табличные разделы не выводились.
3. В контенте MultiPage с блоками разделы с позицией обрабатывались только при `hasContent` по полям; табличные разделы не учитывались.

### Исправление

- В `_buildPositionedDataPage` и `_buildPositionedSectionsForPage`: для разделов с `sectionKind == sectionKindTable` добавлена отдельная ветка — строится виджет таблицы (`_buildTableSectionPdfWidget`) и выводится по настройкам позиции/размера; табличный раздел не пропускается из‑за отсутствия полей.
- В одностраничном потоке блок с виджетами данных (в т.ч. таблицы) добавляется при наличии в шаблоне любых разделов или при непустом `extractedFields`, чтобы таблицы показывались и при пустых данных.
- В `_buildMultiPageOverflowContent`: для табличных разделов добавлена обработка — блок таблицы добавляется в список блоков по странице; условие `hasContent` для таблиц не требуется.
- **Объединения ячеек (mergeRegions):** таблица в PDF переведена с `pw.Table` на `pw.Stack` + `pw.Positioned`: рисуются только «логические» ячейки (позиции, не входящие в объединение как «покрытые», пропускаются); для ячейки, являющейся началом объединения, ширина и высота считаются как сумма столбцов/строк по `rowSpan`/`colSpan`; контент по-прежнему берётся из главной ячейки объединения.

### Связанные файлы

- `lib/features/pdf/services/protocol_pdf_service.dart` (`_buildTableSectionPdfWidget`)
